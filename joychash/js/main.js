function copy(textDiv,btn){let text=$(textDiv).text();var clipboard=new Clipboard(btn,{text:function(){return text;}});clipboard.on('success',function(e){msg('Successfully');});clipboard.on('error',function(e){msg(e);});}
function msg(str,time){if(!time){time=3000;}
...
function showCircuit(num){console.log(num)
...
$(window).scroll(function(){var scrollTop=document.documentElement.scrollTop||document.body.scrollTop;if(scrollTop>18){$('.headNav').addClass('in');}else{$('.headNav').removeClass('in');}
...
$('.headNav a').click(function(){var href=$(this).data('href');$('.headNav a').removeClass('active')
...
return false;});$('.circuitDiv .process li').click(function(event){showCircuit($(this).data('num'));});$('.ruleDiv .tabItem').click(function(event){showRule($(this).data('num'));});$('.exchangeDiv .form button').click(function(event)
{var b1value=$('.exchangeDiv .b1 input').val();if(b1value==''){msg('Please fill in the amount you want to exchange');return;}});$('.whyDiv .buts li').click(function(event){showWhy($(this).data('num'));});$('.faqDiv .list .arrow').click(function(event){$(this).eq().isUp=!$(this).eq().isUp
console.log($(this).eq())
$(this).toggleClass('up')
$(this).parent().next().toggle()});$('.winDiv .colse').click(function(event){$('.winDiv').fadeOut(200);$('.winMask').fadeOut(200);});$('.winMask').click(function(event){$('.winDiv').fadeOut(200);$('.winMask').fadeOut(200);});showCircuit(1);showWhy(1);$('#tipDiv').hide();let exchangeType='usdt_to_trx';$('.countBox button.count').click(function(event)
{var b1value=$('.countBox .b1 input').val();if(b1value==''||b1value==0){msg('Please fill in the amount you want to exchange');return;}
